...
import user_information_mysql as informationMysql
import user_record_mysql as recordMysql
import hand_write_100 as hw
import class_schedule_mysql as scheduleMysql
import zyq
import map
...
def get_login_message(message):
    ...
    if(len(return_password)==0):
        ...
        if(message['password'] == return_password):
            ...
            emit("login_backtips_event", {'data': '用户名或密码错误！'})
            return 0#[0,False]
...
@socketio.on('general_basic_event', namespace='/test')
def tran_img_event(message):
    img = base64.b64decode(message["data"])
    file = open('2.png', "wb")
    file.write(img)
    ...
    for i in  range(len(result["words_result"])):
        str3 +=result["words_result"][i]["words"]
        str3  += "<br/>"
    ...
    if (message['check'] != ""):
        ...
        emit('my_response',
             {'username': '匿名用户', 'time1': time1.strftime('%Y-%m-%d %H:%M:%S'),
              'time2': time2.strftime('%Y-%m-%d %H:%M:%S'), 'data': out})
# 手写汉字识别（百度ai）
@socketio.on('handwriting_event', namespace='/test')
def tran_img_event(message):
    img = base64.b64decode(message["data"])
    file = open('2.png', "wb")
    file.write(img)
    ...
    if (message['check'] != ""):
        ...
        emit('my_response',
             {'username': '匿名用户', 'time1': time1.strftime('%Y-%m-%d %H:%M:%S'),
              'time2': time2.strftime('%Y-%m-%d %H:%M:%S'), 'data': out})
# 公式识别
@socketio.on('formula_event', namespace='/test')
def tran_img_event(message):
    img = base64.b64decode(message["data"])
    file = open('2.png', "wb")
    file.write(img)
    ...
    access_token = '24.cbba50e615a3dbbdc0a88e61c5413823.2592000.1589189383.282335-19375767'
    http = urllib3.PoolManager()
    url = 'https://aip.baidubce.com/rest/2.0/ocr/v1/formula?access_token=' + access_token
    # f = open('2.png', 'wb')  # 图片的路径
    # img = base64.b64encode(f.read())
    imgParam = str(message["data"]).encode('utf-8')
    param = {'image': imgParam, 'show': 'true'}
    ...
    if (message['check'] != ""):
        ...
        emit('my_response',
             {'username': '匿名用户', 'time1': time1.strftime('%Y-%m-%d %H:%M:%S'),
              'time2': time2.strftime('%Y-%m-%d %H:%M:%S'), 'data': out})
# 网络图片文字识别
@socketio.on('webimage_event', namespace='/test')
def tran_img_event(message):
    img = base64.b64decode(message["data"])
    file = open('2.png', "wb")
    file.write(img)
