        ...
        $('form#broadcast').submit(function (event) {
            ...
            if ($('#broadcast_data').val()) {
                ...
                if ((($('#broadcast_data').val()).match("2017级")) || (($('#broadcast_data').val()).match("2018级")) || (($('#broadcast_data').val()).match("2019级"))) {
                    ...
                    var special_ana = ["1.验证码识别2.单个汉字识别3.通用文字识别4.手写文字识别5.公式识别6.网络图片文字识别(只能输入数字1-3哦！)","请输入地址"];
                    var special_que = ["-地图"]
                    //获取人和机器人说的最后一句话
                    var last_robot = "";
                    var last_people = "";
                    ...
                    if(special_ana.indexOf(last_robot) == -1 && last_people.indexOf(special_que[0]) == -1) {
                        ...
                        if(last_people.replace(re,"")=="1") {
                            ...
                            cpoy_src = cpoy_src.substring( cpoy_src.indexOf(",")+1);
                             ...
                             cpoy_src=$(".outgoing-message").eq(-2).children(".message-content1").children("#img_size").attr("src");
                             cpoy_src = cpoy_src.substring( cpoy_src.indexOf(",")+1);
                             socket.emit('general_basic_event', {data: cpoy_src, userId: '123', check: $("#logout_username").text()})
                         }
                         else if(last_people.replace(re,"")=="4"){
                             cpoy_src=$(".outgoing-message").eq(-2).children(".message-content1").children("#img_size").attr("src");
                             cpoy_src = cpoy_src.substring( cpoy_src.indexOf(",")+1);
                             socket.emit('handwriting_event', {data: cpoy_src, userId: '123', check: $("#logout_username").text()})
                         }
                         else if(last_people.replace(re,"")=="5"){
                             cpoy_src=$(".outgoing-message").eq(-2).children(".message-content1").children("#img_size").attr("src");
                             cpoy_src = cpoy_src.substring( cpoy_src.indexOf(",")+1);
                             socket.emit('formula_event', {data: cpoy_src, userId: '123', check: $("#logout_username").text()})
                         }
                         else if(last_people.replace(re,"")=="6"){
                             cpoy_src=$(".outgoing-message").eq(-2).children(".message-content1").children("#img_size").attr("src");
                             cpoy_src = cpoy_src.substring( cpoy_src.indexOf(",")+1);
                             socket.emit('webimage_event', {data: cpoy_src, userId: '123', check: $("#logout_username").text()})
                         }
